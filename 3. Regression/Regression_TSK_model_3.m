...
%% Εκπαίδευση μοντέλου 2 - Hybrid Algorithm TSK με "2 mf" και "Polynomial" output
% Αρχικό Ασαφές Σύστημα
% Επιλέγω linear output (polynomial 1ου βαθμού)
opt_gen = genfisOptions('GridPartition','NumMembershipFunctions',2,'InputMembershipFunctionType', 'gbellmf','OutputMembershipFunctionType','linear');
...
% Calculate Model 3 Evaluations
eval_model_3 = evalfis(TSK_model_3, Dchk(:,1:5));                  % (check data, anfis output)
% Error Metrics
[RMSE, NMSE, NDEI, R2] = calculate(Dchk(:,6), eval_model_3)     % (original output, evaluation output)

% Plot Training Error & Check Error
figure(3)
plot([trainError_3 chkError_3])
grid on
legend('training error', 'check error')
xlabel('EpochNumber')
ylabel('Root Mean Squared Error (RMSE)')
title('Training and Check Error / 2 mf & linear')

% Plot RMSE
mse = (Dchk(:,6)-eval_model_3).^2;
rmse = sqrt(mse);
figure(4)
plot(1:length(eval_model_3), rmse)        
xlabel('check data')
ylabel('Root Mean Square Error (RMSE)')
title('RMSE / 2 mf & linear output')

% Plot Prediction Error
figure(5) 
anfis_output = evalfis(chkFIS_3, d(:,1:5));     % overall data
index = 1:1503;
